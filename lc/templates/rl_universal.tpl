...
<tr><td colspan=8 class=rltop>Список существующих ограничений</td></tr>
<tr>
...
<td><input type=checkbox id="cb[{$v@iteration}]"><input type=hidden id="rid[{$v@iteration}]" value="{$k}"><input type=hidden id="hidd[{$v@iteration}]" value="0"></td>
<td><input type=hidden id="hiddcgrt[{$v@iteration}]" value="{$v.CORG}{$v.RestrictType}">
{($v.RestrictType == "I") ? "Конкретный выпуск" : (($v.RestrictType == "A") ? "Любая бумага" : "Сумма всех бумаг")}   {($v.CORG == "C") ? "эмитента" : "группы"}</td>
<td><input type=hidden id="hiddcgid[{$v@iteration}]" value="{$v.CORGid}">{$v.CORGname}</td>
<td><input type=hidden id="hiddtl[{$v@iteration}]" value="{$v.TypesList}">
<input type=hidden id="hiddiss[{$v@iteration}]" value="{$v.IssueRid}">
{($v.RestrictType == "I") ? $v.Issue : $v.TLNames}</td>
