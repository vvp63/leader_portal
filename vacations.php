...
$query = "  SELECT [ФизическоеЛицоСсылка], [ФизическоеЛицоФИО], [ВидОтпускаНаименование]
            ...
            UNION
            SELECT [ФизическоеЛицоСсылка], [ФизическоеЛицоФИО], 'Командировка' AS [ВидОтпускаНаименование]
                    ,[ДатаНачала], [КоличествоДней], [ДатаОкончания]
            FROM [Обмен1С_Командировки]
                WHERE (([ДатаНачала] < '".date("Ymd", $nextmonth)."') AND ([ДатаОкончания] >= '".date("Ymd", $lastmonth)."')) AND [КоличествоДней] > 0 AND [Проведен] = 1
            UNION
            SELECT [ФизическоеЛицоСсылка], [ФизическоеЛицоФИО], 'Больничный' AS [ВидОтпускаНаименование]
                    ,[ДатаНачала], [КоличествоДней], [ДатаОкончания]
            FROM [Обмен1С_БольничныеЛисты]
                WHERE (([ДатаНачала] < '".date("Ymd", $nextmonth)."') AND ([ДатаОкончания] >= '".date("Ymd", $lastmonth)."')) AND [КоличествоДней] > 0 AND [Проведен] = 1
            ORDER BY [ФизическоеЛицоФИО], [ДатаНачала]";
...
foreach($dbh->query($query) as $row) {
    ...
    $vacantions[$id]["fio"] = $row["ФизическоеЛицоФИО"];
